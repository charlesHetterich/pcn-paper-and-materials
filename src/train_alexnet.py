...
if dataset_name == "cifar10":
    train = datasets.load_data(datasets.Cifar10("train", size=227), batch_size=batch_size, num_workers=num_workers)
    test = datasets.load_data(datasets.Cifar10("test", size=227), batch_size=batch_size, num_workers=num_workers)
else:
    train = datasets.load_data(datasets.Cifar100("train", size=227), batch_size=batch_size, num_workers=num_workers)
    test = datasets.load_data(datasets.Cifar100("test", size=227), batch_size=batch_size, num_workers=num_workers)
...
models[0][1].classifier[6] = torch.nn.Linear(4096, out_features).to(device)
models[1][1].classifier = util.PCN([9216, 4096, 4096, out_features], dimensions=16, dropout=0.5).to(device)
